...
if __name__ == '__main__':
    ...
    with open(f"D://9_Lab//1_Recommendation_Sys//PO4ISR_demo//ALL_dataset//games//expand//train_50_100cand.json", 'r') as json_file:
        train_data = json.load(json_file)
    # with open(f"D:/9_Lab/1_Recommendation_Sys/PO4ISR_demo/Dataset/{conf['dataset']}/Text/valid.json", 'r') as json_file:
    with open(f"D://9_Lab//1_Recommendation_Sys//PO4ISR_demo//ALL_dataset//games//expand//valid_100cand.json", 'r') as json_file:
    ...
    # pbar.close()
    ##Làm phẳng mảng new_candidate, vì output của opt_select.run là 1 mảng 
    ## -> Dẫn tới trường hợp mảng lồng nhau (len(new_candidate) < 5)
    #from itertools import chain
    #new_candidate = list(chain.from_iterable(new_candidate))
